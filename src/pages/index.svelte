...
<div class="block">
  {#if holiday.length > 0}
    <SveltyPicker inputClasses="block mb-4 disabled:cursor-not-allowed disabled:opacity-50 focus:border-blue-500 focus:ring-blue-500 dark:focus:border-blue-500 dark:focus:ring-blue-500 bg-gray-50 text-gray-900 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 border-gray-300 dark:border-gray-600 p-2.5 text-sm rounded-lg" format="yyyy-mm-dd hh:ii" bind:value={date} startDate={moment().format('YYYY-MM-DD')} />
    {#each holiday as h}
      {#if show}
