...
import { FloatingLabelInput, Button, List, Li, Label, Range, Helper, Checkbox, Kbd, Tooltip } from 'flowbite-svelte';
...
function formSubmit() {
  ...
  if (isValid && checkPoint === fields.coordinate) {
...
<div class="flex justify-center">
  <Form class="w-96 h-96" {schema} {fields} submitHandler={formSubmit} {submitted}>
    <List tag="ul" list="none" class="space-y-2 mb-2">
      ...
      <Button on:click={handleRefresh} class="mb-2 w-full my-1" outline color="purple">確認定位</Button>
    </List>
    {#if show}
      <Map {lat} {lng} />
    {/if}
    <Button type="submit" class="w-full my-1" outline color="purple">送出</Button>
    {#if checkPoint !== fields.coordinate}
      <Tooltip color="green">請先點擊「確認定位」刷新地圖</Tooltip>
    {/if}
  </Form>
</div>
